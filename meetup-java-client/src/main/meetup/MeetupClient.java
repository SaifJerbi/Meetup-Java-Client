...
public class MeetupClient {
	...
	protected Response sendApiRequest(java.util.Map<String, String> params,
			String extraPath) {

		String rsp = sendHttpRequest(MEETUP_ENDPOINT_URL, "GET", extraPath,
				params);
		...
		} else if (extraPath.startsWith("member")) {
			clazz = Member.class;
		}
	...
	protected String sendHttpRequest(String baseUrl, String method,
			String extraPath, java.util.Map<String, String> params) {
		...
		if (getUseOAuth()) {
			...
			try {
				...
				Token token = this.getClientSettings()
				...
				OAuthMessage responseMsg = client.invoke(access, method, url,
						params.entrySet());
	...
	protected OAuthServiceProvider createOAuthServiceProvider() {
		...
		OAuthServiceProvider p = new OAuthServiceProvider(
				OAUTH_REQUEST_TOKEN_URL, OAUTH_AUTHORIZE_URL,
	...
	protected OAuthConsumer createOAuthConsumer() {
		...
		OAuthConsumer c = new OAuthConsumer(oauthCallBackUrl,
				consumerToken.getPublicToken(), consumerToken.getSecret(),
	...
	public List<Member> getMembers(MemberSearchCriteria criteria) {
		
		Map<String, String> params = criteria.getParameterMap();
		
		Response r = sendApiRequest(params, "members." + FORMAT);
	...
	public void fetchAccessToken() {
		...
		System.out.println("Consumer token: "
				+ this.getClientSettings().getConsumerToken());
		...
		try {
			...
			responseMsg = client.invoke(access, "GET", OAUTH_ACCESS_TOKEN_URL,
					null);

			this.getUserSpecificAccessToken().setPublicToken(
	...
	public String getUserAuthorizationUrl() {
		...
		try {
			...
			this.requestToken = new Token(access.requestToken,
					access.tokenSecret.trim());
			...
			url = OAUTH_AUTHORIZE_URL + "?oauth_token="
	...
	public List<Activity> getActivities(ActivitySearchCriteria crit) {
		Response r = sendApiRequest(crit.getParameterMap(), "activity."
				+ FORMAT);
	...
	protected HttpClient getHttpClient() {
		...
		if (this.httpClient instanceof DefaultHttpClient) {
			...
			if (this.isCompressionEnabled()) {
				defaultClient.addRequestInterceptor(GzipRequestInterceptor
						.getInstance());
				defaultClient.addResponseInterceptor(GzipResponseInterceptor
						.getInstance());
			}
	...
	public void rsvp(String eventId, YesNoMaybe rsvpSetting, Integer guests,
			String comments, Integer memberId) {
	...
	public void uploadPhoto(String eventId, java.io.InputStream photo,
			String caption) {
